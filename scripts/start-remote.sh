...
# Cassandra-Auth (Bitnami-Helm-Chart erzwingt Auth, Passwort aus k8s-Secret)
CASSANDRA_USERNAME="${CASSANDRA_USERNAME:-cassandra}"
CASSANDRA_PASSWORD="${CASSANDRA_PASSWORD:-$(kubectl get secret -l app.kubernetes.io/name=cassandra -o jsonpath='{.items[0].data.cassandra-password}' 2>/dev/null | base64 -d 2>/dev/null || echo "")}"
