...
This specification describes a comprehensive refactor of the TrustGraph Python API client library to achieve feature
parity with the API Gateway and add support for modern real-time communication patterns.

The refactor addresses four primary use cases:

1. **Streaming LLM Interactions**: Enable real-time streaming of LLM responses (agent, graph RAG, document RAG, text
   completion, prompts) with ~60x lower latency (500ms vs 30s for first token)
2. **Bulk Data Operations**: Support efficient bulk import/export of triples, graph embeddings, and document embeddings
   for large-scale knowledge graph management
3. **Feature Parity**: Ensure every API Gateway endpoint has a corresponding Python API method, including graph
   embeddings query
4. **Persistent Connections**: Enable WebSocket-based communication for multiplexed requests and reduced connection
   overhead

## Goals

- **Feature Parity**: Every Gateway API service has a corresponding Python API method
- **Streaming Support**: All streaming-capable services (agent, RAG, text completion, prompt) support streaming in
  Python API
...
- **Full Async Support**: Complete async/await implementation for all interfaces (REST, WebSocket, bulk operations,
  metrics)
...
The November 2024 streaming enhancement to the Gateway API provided 60x latency improvement (500ms vs 30s first token)
for LLM interactions, but Python API users cannot leverage this capability. This creates a significant experience gap
between Python and non-Python users.

## Technical Design

### Architecture

The refactored Python API uses a **modular interface approach** with separate objects for different communication
patterns. All interfaces are available in both **synchronous and asynchronous** variants:
...
- **Identical signatures**: Where functionality overlaps, method signatures are identical between REST and WebSocket,
  sync and async
- **Progressive enhancement**: Choose interface based on needs (REST for simple, WebSocket for streaming, Bulk for large
  datasets, async for modern frameworks)
...
3. **Asynchronous WebSocket Client**:
    ...
    - `AsyncSocketFlowInstance.text_completion(..., streaming: bool = False)` - Async text completion with optional
      streaming
...
1. **Constructor** (minor enhancement):
   ...
   ```
    ...
    - If specified: Used as bearer token for REST (`Authorization: Bearer <token>`), query param for WebSocket (
      `?token=<token>`)
...
5. **Global Services via WebSocket**: Should `api.socket()` support global services (library, knowledge, collection,
   config) or only flow-scoped services?
    - **Recommendation**: Start with flow-scoped only (where streaming matters). Add global services if needed in Phase
      2.
...
3. **Connection Reuse**: Should `SocketClient` and `BulkClient` share the same WebSocket connection, or use separate
   connections?
