...
This specification describes enhanced command-line configuration capabilities for TrustGraph, enabling users to manage
individual configuration items through granular CLI commands. The integration supports four primary use cases:
...
TrustGraph currently provides configuration management through the Config API and a single CLI command `tg-show-config`
that displays the entire configuration. While this works for viewing configuration, it lacks granular management
capabilities.
...
This specification addresses these gaps by adding four new CLI commands that provide granular configuration management.
By exposing individual Config API operations through CLI commands, TrustGraph can:
